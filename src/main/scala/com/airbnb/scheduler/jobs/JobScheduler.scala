...
//Allows us to let Chaos manage the lifecycle of this class.
  ...
  /* used by withJobLock */
  private val lockSet = new mutable.HashMap[String, (Object, Int)]
  ...
  private def withJobLock(name: String)(block: => Unit) {
    /* modifying lockSet protected by synchronizing on "this" */
    val lock = this.synchronized {
      val (lock, count) = lockSet.getOrElseUpdate(name, (new Object {}, 0))
      lockSet.update(name, (lock, count + 1))
      lock
    }

    /* job manipulation protected by synchronizing on named lock */
    lock.synchronized(block)

    this.synchronized {
      lockSet.get(name) match {
        case None =>
        // don't care... nothing here
        case Some(lockTuple) =>
          val newCount = lockTuple._2 - 1
          if (newCount == 0) {
            lockSet.remove(name)
          } else {
            lockSet.update(name, (lockTuple._1, newCount))
          }
      }
    }
  }
